...
from sjfnw.grants.models import GrantApplication, ProjectApp, GrantAward, GivingProjectGrant
...
class Command(BaseCommand):
  ...
  def handle(self, *args, **kwargs):
    ...
    papps = ProjectApps.objects.all().select_related('application')
    ...
    for award in gp_awards:
      if award.application != award.project_app.application:
        wrong += 1
    if wrong == 0:
      self.print_success()
    else:
